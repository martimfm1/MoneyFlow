...
function calculateHealth(
  ...
  const commitmentScore = Math.round(
    Math.max(0, Math.min(30, (1 - recurringRatio) * 30)),
  )
  return Math.max(
    0,
    Math.min(100, cashFlowScore + savingsScore + commitmentScore),
  )
}
...
export async function MonthlyOverview({ currency }: { currency: string }) {
  ...
  const recurringReserve = (recurring ?? []).reduce((sum, item) => {
    ...
    const divisor =
      item.frequency === 'monthly' ? 1 : item.frequency === 'quarterly' ? 3 : 12
  ...
  return (
    <section className="mt-8 lg:mt-10" aria-labelledby="monthly-overview-title">
      <div className="mb-4">
        ...
        <h2 id="monthly-overview-title" className="mt-1 text-lg font-semibold">
          Visão mensal
        </h2>
      </div>
      <div className="grid gap-3 lg:grid-cols-[1.5fr_1fr]">
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <article className="rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-4 shadow-sm sm:p-5">
            ...
            <p
              className={`mt-2 text-xl font-semibold tabular-nums ${saved < 0 ? 'text-[hsl(var(--danger))]' : 'text-[hsl(var(--success))]'}`}
            >
              {saved >= 0 ? '+' : ''}
        ...
        <article className="rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-4 shadow-sm sm:p-5">
          ...
          <p className="mt-2 text-xs text-[hsl(var(--muted-foreground))]">
            Reserva recorrente mensal detetada:{' '}
            {formatCurrency(recurringReserve, currency)}
          </p>
        </article>
      </div>
    </section>
