...
export default async function WishlistPage({
  ...
  const [{ data: profile }, { data: items }] = await Promise.all([
    ...
    supabase
      .from('wishlist_items')
      .select(
        'id, name, price, category, priority, status, url, image_url, desired_date, notes',
      )
      .eq('user_id', user.id)
          ...
          <section className="mt-6 grid gap-3 sm:grid-cols-3">
            ...
            <article className="rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-5 shadow-sm">
              ...
              <p className="mt-2 text-sm leading-6 text-[hsl(var(--muted-foreground))]">
                Prioridade, estado e data ajudam a decidir sem transformar a
                wishlist numa lista fria de preços.
              </p>
            </article>
          </section>
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 flex flex-wrap gap-1.5 text-xs text-[hsl(var(--muted-foreground))]">
                          <span>
                    ...
                    {item.notes ? (
                      <p className="mt-4 line-clamp-3 text-sm leading-6 text-[hsl(var(--muted-foreground))]">
                        {item.notes}
                      </p>
                    ) : null}
                    {item.desired_date ? (
                      <p className="mt-3 text-xs text-[hsl(var(--muted-foreground))]">
                        Idealmente até{' '}
                        {formatDate(`${item.desired_date}T00:00:00`)}
                      </p>
                    ) : null}
                    <div className="mt-auto space-y-3 pt-5">
                      <form
                        action={updateWishlistStatus}
                      ...
                      >
                        ...
                        <select
                          ...
                          className="min-h-10 min-w-0 flex-1 rounded-[var(--radius-md)] border bg-[hsl(var(--surface))] px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--ring))]"
                        >
