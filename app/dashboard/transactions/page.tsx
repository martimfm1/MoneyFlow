...
import { ArrowDownLeft, ArrowUpRight, Plus, Search, SlidersHorizontal, Tags } from 'lucide-react'
...
export default async function TransactionsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  let query = supabase
    .from('transactions')
    .select('id, transaction_type, amount, description, occurred_at, accounts(name, currency_code), categories(id, name)')
    .eq('user_id', user.id)
      ...
      <section className="mt-6 rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-4 shadow-sm sm:p-5">
        <form className="space-y-3" method="get">
          <label className="relative block">
            ...
            <input name="q" defaultValue={q} placeholder="Pesquisar por descrição" className="min-h-11 w-full rounded-[var(--radius-md)] border bg-transparent pl-9 pr-3 outline-none" />
          </label>
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/dashboard/categories" className="ml-auto text-sm font-medium underline-offset-4 hover:underline">Gerir categorias</Link>
          </div>
        </form>
      </section>
