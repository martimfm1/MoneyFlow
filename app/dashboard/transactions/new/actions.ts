...
export async function createTransaction(formData: FormData) {
  ...
  const { data: account } = await supabase.from('accounts').select('id, is_active').eq('id', parsed.data.accountId).eq('user_id', user.id).maybeSingle()
  ...
  if (categoryId) {
    const { data: category } = await supabase.from('categories').select('id').eq('id', categoryId).eq('user_id', user.id).maybeSingle()
