...
export default async function DashboardPage() {
  ...
  const [{ data: profile }, { data: accounts }, { data: transactions }, { data: goals }] =
    await Promise.all([
      supabase
        .from('profiles')
  ...
  return (
    <main className="min-h-screen pb-24">
      <div className="moneyflow-shell py-6 sm:py-8 lg:py-10">
        ...
        <section className="mt-8 rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-5 shadow-sm sm:p-6 lg:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 text-4xl font-semibold tracking-tight tabular-nums sm:text-5xl lg:text-6xl">
          ...
          {accounts?.length ? (
            <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {accounts.map((account) => (
                <article
                  key={account.id}
                  className="rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
                >
            ...
            <div className="mt-4 rounded-[var(--radius-lg)] border border-dashed bg-[hsl(var(--surface))] p-6 text-center">
              ...
              <Button className="mt-4" asChild><Link href="/dashboard/accounts/new">Criar conta</Link></Button>
            </div>
          )}
        </section>
        ...
        <div className="mt-8 grid gap-8 lg:mt-10 lg:grid-cols-[minmax(0,1.45fr)_minmax(20rem,0.9fr)]">
          <section>
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="text-sm text-[hsl(var(--muted-foreground))]">O que aconteceu</p>
                <h2 className="mt-1 text-lg font-semibold">Movimentos recentes</h2>
              </div>
              <Button variant="ghost" size="sm" asChild>
                <Link href="/dashboard/transactions">Ver todos<ChevronRight className="size-4" /></Link>
              </Button>
            </div>
            ...
            {!transactions?.length ? (
              <div className="mt-4 rounded-[var(--radius-lg)] border border-dashed bg-[hsl(var(--surface))] p-6 text-center">
                ...
                <p className="mt-1 text-sm leading-6 text-[hsl(var(--muted-foreground))]">Regista uma despesa ou receita para começares a ver atividade aqui.</p>
              </div>
            ) : (
              <div className="mt-4 overflow-hidden rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] shadow-sm">
                <div className="divide-y">
                  {transactions.map((transaction) => {
                    const isIncome = transaction.transaction_type === 'income'
                    const account = Array.isArray(transaction.accounts) ? transaction.accounts[0] : transaction.accounts
                    return (
                      <div key={transaction.id} className="flex items-center gap-3 p-4 sm:p-5">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-0.5 truncate text-xs text-[hsl(var(--muted-foreground))]">{account?.name ?? 'Conta'} · {formatShortDate(transaction.occurred_at)}</p>
                        </div>
                        <p className={`shrink-0 text-sm font-semibold tabular-nums ${isIncome ? 'text-[hsl(var(--success))]' : 'text-[hsl(var(--danger))]'}`}>
                          {isIncome ? '+' : '-'}{formatCurrency(Number(transaction.amount), account?.currency_code || currency)}
                        </p>
                      </div>
                    )
                  })}
          ...
          <section>
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="text-sm text-[hsl(var(--muted-foreground))]">Para onde estás a ir</p>
                <h2 className="mt-1 text-lg font-semibold">Objetivos</h2>
              </div>
              <Button variant="ghost" size="sm" asChild>
                <Link href="/dashboard/goals">Ver todos<ChevronRight className="size-4" /></Link>
              </Button>
            </div>
            ...
            {!goals?.length ? (
              <div className="mt-4 rounded-[var(--radius-lg)] border border-dashed bg-[hsl(var(--surface))] p-6 text-center">
                ...
                <p className="mt-1 text-sm leading-6 text-[hsl(var(--muted-foreground))]">Cria uma meta para dar propósito ao dinheiro que estás a guardar.</p>
                <Button asChild className="mt-4"><Link href="/dashboard/goals/new">Criar objetivo</Link></Button>
              </div>
            ) : (
              <div className="mt-4 space-y-3">
                {goals.map((goal) => {
                  ...
                  return (
                    <Link key={goal.id} href={`/dashboard/goals/${goal.id}`} className="block rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-4 transition hover:-translate-y-0.5 hover:shadow-sm">
                      <div className="flex items-center justify-between gap-3">
                        <p className="truncate font-medium">{goal.name}</p>
                        <span className="shrink-0 text-sm font-semibold tabular-nums">{progress}%</span>
                      </div>
                      <div className="mt-3 h-2 overflow-hidden rounded-full bg-[hsl(var(--surface-muted))]"><div className="h-full rounded-full bg-[hsl(var(--primary))] transition-[width]" style={{ width: `${progress}%` }} /></div>
                      <div className="mt-2 flex items-center justify-between text-xs"><span className="tabular-nums">{formatCurrency(current, currency)}</span><span className="text-[hsl(var(--muted-foreground))]">{formatCurrency(target, currency)}</span></div>
                    </Link>
                  )
                })}
              </div>
        ...
        <section id="add" className="mt-8 grid gap-3 sm:grid-cols-2 lg:mt-10 lg:grid-cols-4">
          {[
            ...
            <Link key={title as string} href={href as string} className="rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-4 transition hover:-translate-y-0.5 hover:shadow-sm sm:p-5">
              <span className="inline-flex size-9 items-center justify-center rounded-full bg-[hsl(var(--surface-muted))]"><Icon className="size-4" /></span>
              <p className="mt-3 font-medium">{title as string}</p>
              <p className="mt-1 text-sm leading-6 text-[hsl(var(--muted-foreground))]">{description as string}</p>
            </Link>
          ))}
        </section>
      </div>
