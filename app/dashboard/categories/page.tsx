...
export default async function CategoriesPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <main className="moneyflow-shell py-6 sm:py-10">
      <Link href="/dashboard/transactions" className="inline-flex min-h-11 items-center gap-2 text-sm font-medium text-[hsl(var(--muted-foreground))]">
        <ArrowLeft className="size-4" /> Voltar aos movimentos
      </Link>

      <header className="mt-6">
        <p className="text-sm text-[hsl(var(--muted-foreground))]">Organização</p>
        <h1 className="mt-1 text-2xl font-semibold tracking-tight">Categorias</h1>
        <p className="mt-2 max-w-xl text-sm leading-6 text-[hsl(var(--muted-foreground))]">Mantém as categorias simples para conseguires perceber rapidamente para onde vai o teu dinheiro.</p>
      </header>
      ...
      <section className="mt-6 rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-5 shadow-sm sm:p-6">
        ...
        <form action={createCategory} className="mt-4 flex flex-col gap-3 sm:flex-row">
          ...
          <input id="category-name" name="name" required maxLength={50} placeholder="Ex.: Alimentação" className="min-h-11 flex-1 rounded-[var(--radius-md)] border bg-transparent px-3 outline-none" />
            ...
            {categories.map((category) => (
              <div key={category.id} className="flex items-center justify-between gap-4 p-4">
                <div className="flex items-center gap-3">
                  <span className="inline-flex size-9 items-center justify-center rounded-full bg-[hsl(var(--surface-muted))] text-xs font-semibold">{category.sort_order + 1}</span>
                  <p className="font-medium">{category.name}</p>
                </div>
                <span className="text-xs text-[hsl(var(--muted-foreground))]">Personalizada</span>
              </div>
            ))}
          </div>
        )}
