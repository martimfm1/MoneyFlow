...
export default async function EditRecurringExpensePage({
  ...
  const { data: expense } = await supabase
    ...
    .select(
      'id, name, provider, amount, frequency, next_due_date, currency_code, notes',
    )
    .eq('id', id)
    .eq('user_id', user.id)
    .maybeSingle()
      ...
      <section className="mx-auto mt-8 max-w-3xl rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-5 shadow-sm sm:p-7">
        <header>
          ...
          <p className="mt-2 text-sm leading-6 text-[hsl(var(--muted-foreground))]">
            Atualiza o preço, periodicidade, cliente ou próxima renovação sem
            perder o histórico do registo.
          </p>
        </header>
        ...
        >
          ...
          <label className="grid gap-2 text-sm font-medium">
            ...
            <input
              name="currencyCode"
              defaultValue={expense.currency_code}
              maxLength={3}
              pattern="[A-Z]{3}"
              required
