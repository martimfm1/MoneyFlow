...
export default async function RecurringIncomePage({
  ...
  const [{ data: profile }, { data: incomes }, { data: accounts }] = await Promise.all([
    supabase.from('profiles').select('currency_code').eq('id', user.id).maybeSingle(),
    supabase
      .from('recurring_incomes')
      .select('id, name, source, amount, frequency, next_income_date, currency_code, account_id, account:accounts(name, currency_code), is_active')
      .eq('user_id', user.id)
    ...
    <main className="moneyflow-shell py-6 sm:py-10">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          <Button asChild size="icon" variant="ghost" aria-label="Voltar">
            <Link href="/dashboard/recurring"><ChevronLeft className="size-4" /></Link>
          </Button>
          <div className="min-w-0">
            <p className="text-xs text-muted-foreground">Recorrentes</p>
            <h1 className="truncate text-2xl font-semibold tracking-tight sm:text-3xl">Ganhos</h1>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant="secondary">{items.length}</Badge>
          <RecurringIncomeCreateDialog currency={currency} accounts={accounts ?? []} />
        </div>
      </header>

      {params.error ? (
        <Card className="mt-4 border-destructive/30 bg-destructive/5">
          <CardContent className="pt-4 text-sm text-destructive" role="alert">{params.error}</CardContent>
        </Card>
      ) : null}
      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          ['Próximo', nextIncome ? formatDate(`${nextIncome.next_income_date}T00:00:00`) : '—'],
      ...
      {!items.length ? (
        <Empty className="border bg-card">
          <EmptyHeader>
            <EmptyMedia variant="icon"><ArrowDownLeft className="size-4" /></EmptyMedia>
            <EmptyTitle>Sem ganhos recorrentes</EmptyTitle>
            <EmptyDescription>Adiciona o primeiro rendimento.</EmptyDescription>
          </EmptyHeader>
          <EmptyContent><RecurringIncomeCreateDialog currency={currency} accounts={accounts ?? []} /></EmptyContent>
        </Empty>
      ) : (
        <section>
          <div className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-2"><CalendarClock className="size-4" /><h2 className="text-sm font-semibold">Rendimentos</h2></div>
                  ...
                  <CardHeader className="pb-2">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 truncate text-xs text-muted-foreground">{item.source || 'Sem origem'} · {frequencyLabels[item.frequency]}</p>
                      </div>
                      <RecurringIncomeActionMenu id={item.id} name={item.name} isActive={item.is_active} />
                    </div>
                  </CardHeader>
                  <CardContent>
                    <Badge variant={item.is_active ? 'secondary' : 'outline'}>{item.is_active ? 'Ativo' : 'Pausado'}</Badge>
                    <p className="mt-4 text-2xl font-semibold tabular-nums">{formatCurrency(Number(item.amount), itemCurrency)}</p>
                    <p className="mt-1 text-xs text-muted-foreground">{formatCurrency(monthlyIncome(item), itemCurrency)}/mês</p>
                    <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
                      <div><p className="text-muted-foreground">Conta</p><p className="mt-1 font-medium">{item.account?.name ?? 'Não definida'}</p></div>
                      <div className="text-right"><p className="text-muted-foreground">Próximo</p><p className="mt-1 font-medium">{formatDate(`${item.next_income_date}T00:00:00`)}</p>{item.is_active && days >= 0 && days <= 30 ? <p className="mt-1 text-emerald-600">{days === 0 ? 'Hoje' : `${days}d`}</p> : null}</div>
                    </div>
                  </CardContent>
                </Card>
              )
