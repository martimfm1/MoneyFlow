...
export function RecurringIncomeCreateDialog({ currency, accounts }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger render={<Button />}><Plus className="size-4" /> Novo</DialogTrigger>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader><DialogTitle>Novo ganho recorrente</DialogTitle></DialogHeader>
        <form action={createRecurringIncome} className="grid gap-4">
          <label className="grid gap-2 text-sm font-medium">Nome<Input name="name" required maxLength={120} placeholder="Salário" /></label>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="grid gap-2 text-sm font-medium">Origem<Input name="source" maxLength={120} placeholder="Empresa" /></label>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <FormDatePicker name="nextIncomeDate" defaultValue={new Date().toISOString().slice(0, 10)} label="Próxima entrada" />
          </div>
          <Separator />
