    ...
    <main className="moneyflow-shell py-6 sm:py-10">
      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-3">
        <article className="rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-5 shadow-sm">
          <p className="text-sm text-[hsl(var(--muted-foreground))]">Orçamento</p>
        ...
        <article className="rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-5 shadow-sm">
          <p className="text-sm text-[hsl(var(--muted-foreground))]">Disponível</p>
          <p
      ...
      <section className="mt-8">
        <div>
          <p className="text-sm text-[hsl(var(--muted-foreground))]">Por categoria</p>
        ...
        {!rows.length ? (
          <div className="mt-4 rounded-[var(--radius-lg)] border border-dashed bg-[hsl(var(--surface))] p-8 text-center">
            ...
            <h3 className="mt-4 font-medium">Ainda não existem dados neste mês</h3>
            <p className="mx-auto mt-1 max-w-md text-sm leading-6 text-[hsl(var(--muted-foreground))]">
              Cria um orçamento por categoria para começares a acompanhar os teus limites mensais.
            </p>
          </div>
        ) : (
          <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {rows.map((row) => {
              ...
              const percentage = budget > 0 ? Math.round((row.spent / budget) * 100) : 100
              const clamped = Math.min(100, percentage)
              const over = row.spent > budget && row.budget !== null
              return (
                <article key={row.id} className="rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h3 className="font-medium">{row.name}</h3>
                      <p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">
                        {row.budget === null ? 'Sem orçamento definido' : `Limite ${formatCurrency(budget, currency)}`}
                      </p>
                    </div>
                    <span className={`text-sm font-semibold tabular-nums ${over ? 'text-[hsl(var(--danger))]' : ''}`}>
                      {row.budget === null ? '—' : `${percentage}%`}
                    </span>
                  </div>
                  <div className="mt-4 h-2 overflow-hidden rounded-full bg-[hsl(var(--surface-muted))]">
                    <div className={`h-full rounded-full ${over ? 'bg-[hsl(var(--danger))]' : 'bg-[hsl(var(--primary))]'}`} style={{ width: `${clamped}%` }} />
                  </div>
                  <div className="mt-3 flex items-center justify-between gap-3 text-sm">
                    <span className="tabular-nums">{formatCurrency(row.spent, currency)} gastos</span>
                    {row.budget !== null ? (
                      <span className={`tabular-nums ${over ? 'font-medium text-[hsl(var(--danger))]' : 'text-[hsl(var(--muted-foreground))]'}`}>
                        {over ? `+${formatCurrency(row.spent - budget, currency)}` : `${formatCurrency(Math.max(0, budget - row.spent), currency)} restantes`}
                      </span>
                    ) : null}
                  </div>
                </article>
      ...
      <section id="novo-orcamento" className="mt-8 rounded-[var(--radius-lg)] border bg-[hsl(var(--surface))] p-5 shadow-sm sm:p-6">
        <div>
          <p className="text-sm text-[hsl(var(--muted-foreground))]">Configuração</p>
          <h2 className="mt-1 text-lg font-semibold">Definir orçamento</h2>
          <p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">Guardar outra vez a mesma categoria neste mês atualiza o limite existente.</p>
        </div>
        {availableCategories.length ? (
          <form action={upsertBudget} className="mt-5 grid gap-3 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
            <input type="hidden" name="month" value={currentMonth} />
            <label className="grid gap-2 text-sm font-medium">
              Categoria
              <select name="categoryId" required className="min-h-11 rounded-[var(--radius-md)] border bg-transparent px-3 font-normal outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--ring))]">
            ...
            <label className="grid gap-2 text-sm font-medium">
              Limite mensal
              <input name="amount" type="text" inputMode="decimal" autoComplete="off" placeholder="Ex.: 150,00" required className="min-h-11 rounded-[var(--radius-md)] border bg-transparent px-3 font-normal outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--ring))]" />
            </label>
          ...
          <div className="mt-5 flex flex-col gap-3 rounded-xl border border-dashed p-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-[hsl(var(--muted-foreground))]">Cria primeiro uma categoria para poderes definir um orçamento.</p>
            <Button asChild variant="outline"><Link href="/dashboard/categories"><Plus className="size-4" /> Criar categoria</Link></Button>
          </div>
        )}
      </section>
    </main>
